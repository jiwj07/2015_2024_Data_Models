...
# printing data
df = df.drop(columns = ['Country', 'River', 'Land Area'])
...
results = []
results.append(("Perceptron", "None", "None", mse, mae))
...
feature_selectors = {
    ...
    "RFE1": RFE(perceptron, n_features_to_select=1),
    "RFE2": RFE(perceptron, n_features_to_select=2),
    "RFE3": RFE(perceptron, n_features_to_select=3),
...
for (fs_name, fs) in feature_selectors.items():
    ...
    # Predictions
    y_pred = perceptron.predict(X_test_fs)
    print()
    ...
    results.append(("Perceptron", fs_name, "None", mse, mae))
...
for (fs1_name, fs1) in feature_selectors1.items():
    ...
    for(fs2_name, fs2) in feature_selectors2.items(): 
        ...
        # Train model
        perceptron.fit(X_train_fs2, y_train)
        
        # Predictions
        y_pred = perceptron.predict(X_test_fs2)
        print()
        ...
        mse = mean_squared_error(y_test, y_pred)
        mae = mean_absolute_error(y_test, y_pred)
        results.append(("Perceptron", fs1_name, fs2_name, mse, mae))
